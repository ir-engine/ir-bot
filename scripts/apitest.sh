#!/bin/bash
# run with local deployment of ethereal engine 
curl -i -X GET http://localhost:4000/
echo -e "\nadding bot\n"
curl -i -X PUT -H 'Content-Type: application/json' -d '{"name":"Rahul Ghosh","options":{"verbose":false,"headless":false}}' http://localhost:4000/bots/07032002/create
echo -e "\nfetching bot\n"
